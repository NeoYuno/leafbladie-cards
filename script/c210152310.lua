...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>0
		and Duel.IsPlayerCanSpecialSummonMonster(p,210152314,0,0,1000,1000,1,RACE_PYRO,ATTRIBUTE_FIRE) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
    local p=Duel.GetTurnPlayer()
	local ft=Duel.GetLocationCount(p,LOCATION_MZONE)
	if ft<=0 or not Duel.IsPlayerCanSpecialSummonMonster(p,210152314,0,0,1000,1000,1,RACE_PYRO,ATTRIBUTE_FIRE) then return end
	if Duel.IsPlayerAffectedByEffect(p,CARD_BLUEEYES_SPIRIT) then ft=1 end
	local fid=e:GetHandler():GetFieldID()
	for i=1,ft do
		local token=Duel.CreateToken(p,210152314)
