...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return aux.SelectUnselectGroup(cg,e,tp,5,5,s.rescon,0) and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil,#cg)
	end
	local cg=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_MZONE,0,nil)
	local tg=Duel.GetMatchingGroup(s.thfilter,tp,LOCATION_DECK,0,nil,#cg)
	local rg1=Group.CreateGroup()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local rg2=aux.SelectUnselectGroup(cg,e,tp,5,5,s.rescon,1,tp,HINTMSG_REMOVE,cancelcon,breakcon)
	rg1:Merge(rg2)
